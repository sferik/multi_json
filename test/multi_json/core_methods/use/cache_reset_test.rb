...
class UseCacheResetTest < Minitest::Test
  ...
  def test_use_calls_options_cache_reset_not_nil
    ...
    MultiJSON::OptionsCache.generate.fetch(key) { "cached_before" }

    assert_equal "cached_before", MultiJSON::OptionsCache.generate.fetch(key, nil)

    MultiJSON.use(:json_gem)

    # After use, cache should be cleared
    # If OptionsCache.reset was replaced with nil, cache would NOT be cleared
    assert_nil MultiJSON::OptionsCache.generate.fetch(key, nil), "Cache should be cleared after use"
  end
  ...
  def test_use_preserves_cache_when_load_adapter_raises
    ...
    # failed swap.
    assert_equal "cached", MultiJSON::OptionsCache.generate.fetch(key, nil)
  end
end
